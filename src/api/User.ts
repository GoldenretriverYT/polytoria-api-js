...
export class Users {
  ...
  static async getUsers(
    ...
    if (options.limit && options.limit > 100) {
      throw new Error(
        "Limit cannot be greater than 100. Use Users.getManyUsers instead."
      );
    }

    const body = (await polyFetch(
      "https://api.polytoria.com/v1/users?" +
  ...
  static async getFriends(options: FriendsOptions): Promise<FriendsResponse> {
    const body = (await polyFetch(
      `https://api.polytoria.com/v1/users/${options.id}/friends?` +
  ...
  static async getUserBadges(
    ...
    const body = (await polyFetch(
      `https://api.polytoria.com/v1/users/${options.id}/badges?` +
        new URLSearchParams(options as any),
      {
        method: "GET",
        headers: {
  ...
  static async getManyUsers(
    options: Omit<UserSearchOptions, "page"> & { limit: number }
  ): Promise<UserSearchResponse> {
    if (!PolytoriaAPI.handleRatelimits)
      throw new Error(
        "Cannot use getManyUsers without PolytoriaAPI.handleRatelimits enabled."
      );
   ...
   */
  static async getUsersWithCondition(
    nUsers: number,
    searchOptions: Omit<UserSearchOptions, "page" | "limit">,
    condition: (user: UserSearchResponseUser) => boolean
  ): Promise<UserSearchResponse> {
    if (!PolytoriaAPI.handleRatelimits)
      throw new Error(
        "Cannot use getUsersWithCondition without PolytoriaAPI.handleRatelimits enabled."
      );
  ...
  static async getUsersUntilCondition(
    searchOptions: Omit<UserSearchOptions, "page" | "limit">,
    condition: (user: UserSearchResponseUser) => boolean
  ): Promise<UserSearchResponse> {
    if (!PolytoriaAPI.handleRatelimits)
      throw new Error(
        "Cannot use getUsersUntilCondition without PolytoriaAPI.handleRatelimits enabled."
      );
  ...
  static async getLeaderboard(
    type: "networth" | "visits" | "sales" | "forumposts" | "profileviews",
    page: number
  ) {
    if (!PolytoriaAPI.ptAuthCookie)
      throw new Error(
        "Cannot use getLeaderboard without a valid Polytoria cookie."
      );

    const body = (await polyFetch(
      `https://polytoria.com/api/rankings?category=${type}&page=${page}`,
      {
        method: "GET",
        headers: {
